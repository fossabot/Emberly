  ...
  try {
    ...
    if (slug) {
      if (searchParams.get('admin') === 'true') {
        ...
        if (
          !user ||
          !hasPermission(user.role as any, Permission.MANAGE_SETTINGS)
        ) {
...
export async function POST(request: Request) {
  try {
    ...
    const { slug, title, content, excerpt, status, publishedAt, sortOrder } =
      body || {}
    ...
    return apiResponse(created, HTTP_STATUS.CREATED)
  } catch (error) {
    if (
      error instanceof Error &&
      error.message === 'Invalid legal page status'
    ) {
