  ...
  try {
    ...
    if (!state || !storedState || state !== storedState) {
      const response = NextResponse.redirect(
        new URL('/me?error=Invalid%20GitHub%20OAuth%20state', baseUrl)
      )
    ...
    if (!tokenData.access_token) {
      console.error(
        '[GitHub OAuth callback] Token exchange failed:',
        tokenData.error
      )
      return NextResponse.redirect(
        new URL(
    ...
    const response = NextResponse.redirect(
      new URL('/me?success=GitHub%20account%20linked', baseUrl)
    )
    response.cookies.delete('github_oauth_state')
    return response
  } catch (error) {
