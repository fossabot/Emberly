...
function PodiumCard({
  ...
  const colors = {
    ...
    3: {
      ...
      glow: 'from-amber-700 to-amber-900',
      badge: 'bg-amber-800/10 text-amber-700 dark:text-amber-500',
    },
  }[rank] ?? { ring: '', glow: '', badge: '' }

  return (
    <div
      className={`glass-card h-full transform hover:-translate-y-2 transition-all duration-300 relative overflow-visible ${isFirst ? 'gradient-border-animated z-20' : 'glass-hover z-10'}`}
    >
      {isFirst && (
        <div className="absolute inset-0 bg-gradient-to-b from-primary/5 to-transparent pointer-events-none rounded-2xl" />
      )}
      <div
        className={`absolute ${isFirst ? '-top-8' : '-top-6'} left-1/2 -translate-x-1/2 z-20`}
      >
        <RankBadge rank={rank} />
      </div>
      <div
        className={`${isFirst ? 'p-8 pt-12' : 'p-6 pt-10'} text-center flex flex-col items-center h-full`}
      >
        <div
          className={`relative ${isFirst ? 'w-24 h-24' : 'w-20 h-20'} mb-4 overflow-hidden rounded-full`}
        >
          {isFirst && (
          ...
          {user.image ? (
            ...
            <div
              className={`${isFirst ? 'relative z-10' : ''} w-full h-full rounded-full bg-gradient-to-br ${colors.glow} flex items-center justify-center text-white ${isFirst ? 'text-3xl' : 'text-2xl'} font-bold`}
            >
        ...
        <h3
          className={`${isFirst ? 'text-2xl' : 'text-xl'} font-bold mb-1 truncate w-full`}
        >
          {user.name || 'Anonymous'}
        </h3>
        <p className="text-sm text-muted-foreground mb-3">@{user.name}</p>
        <div
          className={`mt-auto inline-flex items-center gap-2 px-3 py-1.5 rounded-full ${colors.badge} font-medium text-sm`}
        >
          {stat} {statLabel}
        </div>
      </div>
...
function ContributorRow({ user }: { user: ContributorUser }) {
  return (
    <Link href={`/user/${user.name}`} className="block group">
      <div className="flex items-center gap-3 px-3 py-2.5 rounded-lg glass-subtle glass-hover transition-all">
        ...
        <div className="flex-shrink-0">
          {user.image ? (
            ...
            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-primary/20 to-primary/5 flex items-center justify-center border border-primary/20 text-primary font-bold">
              {user.name?.charAt(0).toUpperCase() || '?'}
            </div>
          )}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5">
            ...
            {user.alphaUser && (
              <Badge
                variant="outline"
                className="text-[9px] h-3.5 px-1 bg-amber-500/10 border-amber-500/20 text-amber-500 shrink-0"
              >
                Alpha
              </Badge>
            )}
      ...
      <div className="flex items-center gap-3 px-3 py-2.5 rounded-lg glass-subtle glass-hover transition-all">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5">
            ...
            <Badge
              variant="outline"
              className={`text-[9px] h-3.5 px-1 shrink-0 ${availColor[user.availability] ?? ''}`}
            >
              {user.availability}
            </Badge>
          </div>
        ...
        <div className="text-right flex-shrink-0 flex items-center gap-2">
          <div className="text-right hidden sm:block">
            ...
            <span className="block text-xs text-muted-foreground">skills</span>
          </div>
...
function SquadRow({ squad }: { squad: Squad }) {
  return (
    <div className="flex items-center gap-3 px-3 py-2.5 rounded-lg glass-subtle glass-hover transition-all">
      ...
      <div className="flex-shrink-0">
        <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-primary/20 to-primary/5 flex items-center justify-center border border-primary/20 text-primary font-bold">
  ...
  const LoadingSkeleton = () => (
    <div className="space-y-4">
      {Array.from({ length: 5 }).map((_, i) => (
        <div
          key={i}
          className="flex items-center gap-4 p-4 rounded-xl glass-subtle animate-pulse"
        >
    ...
    <div className="pb-32">
      ...
      <div className="text-center mb-12 space-y-4 animate-fade-up">
        ...
        <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
          Recognizing the most active contributors, discovery profiles, and
          squads in the community.
        </p>
      </div>
      ...
      >
        ...
        <TabsContent value="contributors" className="space-y-8">
          {!loading && contributors.length >= 3 && (
            <div className="hidden md:grid grid-cols-3 gap-6 items-end animate-fade-up">
              <Link
                href={`/user/${contributors[1].name}`}
                className="group h-full"
              >
                <PodiumCard
                  user={contributors[1]}
                  rank={2}
                  stat={contributors[1].fileCount.toLocaleString()}
                  statLabel="Files"
                />
              </Link>
              <Link
                href={`/user/${contributors[0].name}`}
                className="group h-full -mt-12"
              >
                <PodiumCard
                  user={contributors[0]}
                  rank={1}
                  stat={contributors[0].fileCount.toLocaleString()}
                  statLabel="Files"
                />
              </Link>
              <Link
                href={`/user/${contributors[2].name}`}
                className="group h-full"
              >
                <PodiumCard
                  user={contributors[2]}
                  rank={3}
                  stat={contributors[2].fileCount.toLocaleString()}
                  statLabel="Files"
                />
              </Link>
            </div>
        ...
        <TabsContent value="discovery" className="space-y-8">
          {!loading && discoveryUsers.length >= 3 && (
            <div className="hidden md:grid grid-cols-3 gap-6 items-end animate-fade-up">
              <Link
                href={`/user/${discoveryUsers[1].name}`}
                className="group h-full"
              >
                <PodiumCard
                  user={discoveryUsers[1]}
                  rank={2}
                  stat={`${discoveryUsers[1].skillCount} skills`}
                  statLabel=""
                />
              </Link>
              <Link
                href={`/user/${discoveryUsers[0].name}`}
                className="group h-full -mt-12"
              >
                <PodiumCard
                  user={discoveryUsers[0]}
                  rank={1}
                  stat={`${discoveryUsers[0].skillCount} skills`}
                  statLabel=""
                />
              </Link>
              <Link
                href={`/user/${discoveryUsers[2].name}`}
                className="group h-full"
              >
                <PodiumCard
                  user={discoveryUsers[2]}
                  rank={3}
                  stat={`${discoveryUsers[2].skillCount} skills`}
                  statLabel=""
                />
              </Link>
            </div>
