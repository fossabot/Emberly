...
function SkillLevelBar({ level }: { level: string }) {
  ...
  return (
    <div
      ...
      title={
        NEXIUM_SKILL_LEVEL_LABELS[
          level as keyof typeof NEXIUM_SKILL_LEVEL_LABELS
        ]
      }
    >
      {levels.map((_, i) => (
        <div
          key={i}
          className={`w-1.5 h-1.5 rounded-full ${i <= rank ? colorMap[level] : 'bg-muted-foreground/20'}`}
        />
      ))}
    </div>
  )
...
export function PublicProfile({
  ...
  const hasDiscordBooster = user.perkRoles.some((p) =>
    p.startsWith('DISCORD_BOOSTER')
  )
  ...
  const CONTRIBUTOR_TIER_STYLES: Record<
    string,
    { className: string; gradient: string }
  > = {
    Bronze: {
      className: 'text-amber-700 dark:text-amber-500 border-amber-600/40',
      gradient: 'from-amber-800/30 via-amber-700/20 to-amber-600/10',
    },
    Silver: {
      className: 'text-slate-400 dark:text-slate-300 border-slate-400/40',
      gradient: 'from-slate-500/30 via-slate-400/20 to-slate-300/10',
    },
    Gold: {
      className: 'text-yellow-500 dark:text-yellow-400 border-yellow-500/40',
      gradient: 'from-yellow-600/30 via-yellow-500/20 to-yellow-300/10',
    },
    Platinum: {
      className: 'text-cyan-400 dark:text-cyan-300 border-cyan-400/40',
      gradient: 'from-cyan-500/30 via-cyan-400/20 to-cyan-300/10',
    },
    Diamond: {
      className: 'text-sky-300 dark:text-sky-200 border-sky-400/50',
      gradient: 'from-sky-500/40 via-indigo-400/25 to-violet-400/15',
    },
  }

  const BOOSTER_TIER_STYLES: Record<
    string,
    { className: string; gradient: string }
  > = {
    Bronze: {
    ...
    Platinum: {
      ...
      gradient: 'from-fuchsia-600/30 via-purple-500/20 to-pink-400/10',
    },
    Diamond: {
      className: 'text-purple-300 dark:text-purple-200 border-purple-400/50',
      gradient: 'from-purple-600/40 via-fuchsia-500/25 to-pink-400/15',
    },
  }

  if (hasContributor && contributorInfo) {
    const style =
      CONTRIBUTOR_TIER_STYLES[contributorInfo.tier] ??
      CONTRIBUTOR_TIER_STYLES.Bronze
    badges.push({
  ...
  if (hasDiscordBooster && boosterInfo) {
    const style =
      BOOSTER_TIER_STYLES[boosterInfo.tier] ?? BOOSTER_TIER_STYLES.Bronze
    badges.push({
  ...
  if (hasAffiliate) {
    badges.push({
      ...
      className: 'text-blue-500 dark:text-blue-400 border-blue-500/40',
      gradient: 'from-blue-600/25 via-blue-500/15 to-blue-400/5',
    })
  }
  if (user.role === 'SUPERADMIN') {
    badges.push({
      ...
      className: 'text-red-500 dark:text-red-400 border-red-500/40',
      gradient: 'from-red-600/25 via-red-500/15 to-orange-400/5',
    })
  } else if (user.role === 'ADMIN') {
    badges.push({
      label: 'Admin',
      icon: <Shield className="w-3 h-3" />,
      className: 'text-orange-500 dark:text-orange-400 border-orange-500/40',
      gradient: 'from-orange-600/25 via-orange-500/15 to-amber-400/5',
    })
  }
  if (user.alphaUser) {
    badges.push({
      ...
      gradient: 'from-amber-500/25 via-yellow-400/15 to-amber-300/5',
    })
  }
    ...
    <HomeShell>
      <div className="space-y-4 sm:space-y-6">
        ...
        <GlassCard>
          ...
          <div
            className={`px-6 md:px-8 pb-6 ${user.banner ? '-mt-16 relative z-10' : 'pt-6'}`}
          >
              ...
              <div className="flex-1 min-w-0 pt-2">
                <div className="flex items-center gap-2.5 flex-wrap mb-1">
                  ...
                  {user.isVerified && (
                    <Badge
                      variant="outline"
                      className="text-blue-600 dark:text-blue-400 border-blue-500/30 bg-blue-500/10 gap-1 py-0.5 px-2 text-xs"
                    >
                  ...
                  {leaderboardRank && (
                    <Link href="/leaderboard">
                      <Badge
                        variant="outline"
                        className="text-amber-600 dark:text-amber-400 border-amber-500/30 bg-amber-500/10 hover:bg-amber-500/20 cursor-pointer gap-1 py-0.5 px-2 text-xs"
                      >
                ...
                <div className="flex items-center gap-3 text-xs text-muted-foreground mt-2 flex-wrap">
                  ...
                  {nexiumProfile?.timezone && (
                    <span className="flex items-center gap-1.5">
                      <Globe className="w-3.5 h-3.5" /> {nexiumProfile.timezone}
                    </span>
                  )}
                  ...
                  {nexiumProfile && (
                    <Badge
                      variant="outline"
                      className={`text-xs gap-1 py-0 px-2 ${availabilityStyle[nexiumProfile.availability] ?? ''}`}
                    >
                      {
                        NEXIUM_AVAILABILITY_LABELS[
                          nexiumProfile.availability as keyof typeof NEXIUM_AVAILABILITY_LABELS
                        ]
                      }
                    </Badge>
                  )}
                ...
                {nexiumProfile?.headline && !user.bio && (
                  <p className="text-sm text-foreground/80 mt-3 leading-relaxed">
                    {nexiumProfile.headline}
                  </p>
                )}
              </div>
            ...
            <div className="mt-4 flex flex-wrap gap-2 items-center">
              ...
              {(linkedAccounts?.github || user.github) && (
                <a
                  href={`https://github.com/${linkedAccounts?.github || user.github}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-border/50 bg-background/50 text-xs hover:border-primary/30 transition-colors"
                >
            ...
            {badges.length > 0 && (
              <div className="flex flex-wrap gap-2 mt-4">
                {badges.map((b) => (
                  <div
                    key={b.label}
                    className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg border text-xs font-medium bg-gradient-to-r ${b.gradient ?? ''} ${b.className}`}
                  >
                    {b.icon}
                    <span>{b.label}</span>
                  </div>
            ...
            {canReport && (
              <Button
                variant="ghost"
                size="sm"
                className="text-muted-foreground hover:text-destructive sm:hidden mt-3"
                onClick={() => setReportOpen(true)}
              >
            ...
            <GlassCard>
              <div className="p-4 text-center">
                <div className="text-2xl font-bold text-emerald-600 dark:text-emerald-400">
                  {contributorInfo.linesOfCode.toLocaleString()}
                </div>
                <p className="text-xs text-muted-foreground mt-0.5">
                  Lines Contributed
                </p>
              ...
              <GlassCard>
                <div className="p-5">
                  ...
                  <div className="space-y-4">
                    {Object.entries(
                      nexiumProfile!.skills.reduce<
                        Record<string, typeof nexiumProfile.skills>
                      >((acc, s) => {
                      ...
                      <div key={cat}>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {catSkills.map((skill) => (
                            <div
                              key={skill.id}
                              className="flex items-center gap-2 px-3 py-2 rounded-xl border border-border/50 bg-background/40 hover:bg-background/60 transition-colors"
                            >
                              <SkillIcon
                                name={skill.name}
                              ...
                              {skill.yearsExperience != null && (
                                <span className="text-[10px] text-muted-foreground border-l border-border/50 pl-1.5">
                ...
                <GlassCard>
                  <div className="p-5">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      {contributions.stats && (
                        <div className="flex items-center gap-3 text-xs text-muted-foreground">
                          ...
                          <span>
                            {contributions.stats.totalRepos} repo
                            {contributions.stats.totalRepos !== 1 ? 's' : ''}
                          </span>
                        </div>
                      )}
                    </div>
                    ...
                    {contributions.recentCommits &&
                      contributions.recentCommits.length > 0 && (
                        <div className="space-y-1">
                          {contributions.recentCommits
                            .slice(0, 8)
                            .map((commit: any) => (
                              ...
                              >
                                <code className="text-[10px] font-mono text-muted-foreground shrink-0 group-hover:text-primary transition-colors">
                                  {commit.sha}
                                </code>
